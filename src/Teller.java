...
public class Teller
{
                            ...
                            Report toReportTo)
    {
        ...
        SimulationEvent nextGeneration = new CheckForCustomerEvent(
                theEventQueue.getCurrentTime(), "Generate the first customer.");
        theEventQueue.add(nextGeneration);

        //Customer customer = theLine.getFront(); //get the initial event.
    ...
    public class CheckForCustomerEvent extends SimulationEvent
    {
    	...
    	public void process()
    	{
            if(theLine.isEmpty())
            {
                ...
                SimulationEvent next = new CheckForCustomerEvent(theEventQueue.getCurrentTime()+1,"Check line for customer");

                theEventQueue.add(next);
                serving = null; //step 15
                postActionReport = "No customers to serve."; //step 16

            }
            else
            {
                Customer toHelp = theLine.dequeue();
                serve(toHelp);

                int timeToNext = sharedRandomGenerator.nextInt(maxForHelp);

                SimulationEvent nextGeneration = new CheckForCustomerEvent(theEventQueue.getCurrentTime() + timeToNext
